...
def render(df: pd.DataFrame, out_path: str):
    ...
    df_sorted = df_sorted.sort_values(["__rank", "__chg_key"], ascending=[True, True]).drop(
        columns=["__rank", "__chg_key"]
    )
...
<tr class="{tr_cls}">
  ...
  <td class="col-link">
    <a class="btn" href="{quote_url}" target="_blank" rel="noopener">詳細</a>
  </td>
</tr>
"""
        )
